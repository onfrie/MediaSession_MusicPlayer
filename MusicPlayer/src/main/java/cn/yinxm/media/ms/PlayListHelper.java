...
public class PlayListHelper {
    ...
    public static List<PlayBean> getPlayList() {
        ...
        playBean.mediaId = String.valueOf(R.raw.inspire);
        playBean.tilte = "Inspire.mp3";
        playBean.artist = "test";
        list.add(playBean);

        PlayBean playBean2 = new PlayBean();
        playBean2.mediaId = String.valueOf(R.raw.gydgs);
     ...
     * 获取扫描的音乐列表
     */
    public static List<PlayBean> getScannedPlayList(Context context) {
        List<PlayBean> list = new ArrayList<>();

        // 添加内置的示例音乐
        list.addAll(getPlayList());
        ...
        try {
            ...
            Log.e("PlayListHelper", "扫描音乐文件时出错", e);
        }
    ...
    public static MediaMetadataCompat transformPlayBean(PlayBean bean) {
        MediaMetadataCompat metadata = new MediaMetadataCompat.Builder()
                ...
                .putString(MediaMetadataCompat.METADATA_KEY_ALBUM, "本地音乐")
                .putString(MediaMetadataCompat.METADATA_KEY_GENRE, "Pop")
                .build();

        Log.d("PlayListHelper", "创建MediaMetadata: " + bean.tilte + " - " + bean.artist);
        return metadata;
    }
